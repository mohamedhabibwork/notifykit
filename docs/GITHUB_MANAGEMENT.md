...
| Release             | `v*` tags or manual dispatch           | Verifies the project, publishes to npm, and attaches signed provenance.                |
| Dependabot          | Weekly                                 | Opens updates for npm dependencies and GitHub Actions.                                 |

## Release process

1. Update `package.json` with the intended semantic version and update the release notes.
...
5. Watch the **Release** workflow. It publishes with `npm publish --provenance`.
6. Confirm the version on npm and create GitHub release notes if desired.
